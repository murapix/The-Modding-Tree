...
// data = a function returning the content (actually HTML)
	...
	Vue.component('buyable', {
		...
		<div v-if="tmp[layer].buyables && tmp[layer].buyables[data] !== undefined && tmp[layer].buyables[data].unlocked" style="display: grid">
			<button v-bind:class="{ buyable: true, tooltipBox: true, can: tmp[layer].buyables[data].canBuy, locked: !tmp[layer].buyables[data].canAfford, bought: new Decimal(player[layer].buyables[data]).gte(tmp[layer].buyables[data].purchaseLimit)}"
