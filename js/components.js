		...
		template: `
			<div>
				<div v-if="tmp[layer].buyables && tmp[layer].buyables.respec && !(tmp[layer].buyables.showRespec !== undefined && tmp[layer].buyables.showRespec == false)">
					<div v-if="tmp[layer].buyables.respecConfirm === undefined || tmp[layer].buyables.respecConfirm" class="tooltipBox"><input type="checkbox" v-model="player[layer].noRespecConfirm" ><tooltip v-bind:text="'Disable respec confirmation'"></tooltip></div>
